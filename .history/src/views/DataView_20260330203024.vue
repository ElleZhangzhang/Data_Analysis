...
<template>
  <button @click="console.log(currentDataset)">currentDataset</button>
...
const dataStore = useDataStore();
const { currentDataset, datasets, addDataset, setCurrentDataset } = dataStore;

// console.log("rows:", currentDataset.value?.rows);
...
const currentCharts = computed(() => {
  ...
  console.log("currentDataset更新了");

  return dashboardStore.getChartsByDataset(currentDataset.value.id);
});
console.log("currentCharts", currentCharts.value);
