  ...
  switch (config.type) {
        ...
        series: [
          {
            ...
            type: "pie" as const,
            radius: "60%",
            data: Array.from(dataMap.entries()).map(([name, value]) => ({
              name,
              value,
            })),
            emphasis: {
    ...
    case "scatter": {
      ...
      return {
        ...
        grid: gridOpt,
        xAxis: { type: "value" as const, name: config.xAxis },
        yAxis: { type: "value" as const, name: config.yAxis },
        series: [
          {
  ...
  >
    ...
    <div v-else class="chart-embed-body">
      ...
      <div class="chart-embed-header">
        ...
        <button v-if="selected" class="embed-delete-btn" @click="deleteNode">
          删除
        </button>
      </div>
    </div>
